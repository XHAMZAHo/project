...
@section('title', app()->getLocale()==='ar' ? 'إضافة سؤال' : 'Add FAQ')
@section('page-title', app()->getLocale()==='ar' ? 'إضافة سؤال جديد' : 'Add New FAQ')
    ...
    <form method="POST" action="{{ route('admin.faqs.store') }}">
        ...
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:20px;margin-bottom:20px;">
            <div>
                ...
                <input type="text" name="question_ar" value="{{ old('question_ar') }}" required style="width:100%;background:var(--bg-base);border:1px solid var(--border);border-radius:10px;padding:12px 14px;color:#fff;outline:none;">
            </div>
            <div>
                ...
                <input type="text" name="question_en" value="{{ old('question_en') }}" required style="width:100%;background:var(--bg-base);border:1px solid var(--border);border-radius:10px;padding:12px 14px;color:#fff;outline:none;">
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:20px;margin-bottom:20px;">
            <div>
                ...
                <textarea name="answer_ar" rows="4" required style="width:100%;background:var(--bg-base);border:1px solid var(--border);border-radius:10px;padding:12px 14px;color:#fff;outline:none;resize:vertical;">{{ old('answer_ar') }}</textarea>
            </div>
            <div>
                ...
                <textarea name="answer_en" rows="4" required style="width:100%;background:var(--bg-base);border:1px solid var(--border);border-radius:10px;padding:12px 14px;color:#fff;outline:none;resize:vertical;">{{ old('answer_en') }}</textarea>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:20px;margin-bottom:30px;">
            <div>
                ...
                <select name="category" style="width:100%;background:var(--bg-base);border:1px solid var(--border);border-radius:10px;padding:12px 14px;color:#fff;outline:none;appearance:none;">
                    <option value="general" {{ old('category')=='general'?'selected':'' }}>{{ app()->getLocale()==='ar'?'عام':'General' }}</option>
                    <option value="services" {{ old('category')=='services'?'selected':'' }}>{{ app()->getLocale()==='ar'?'الخدمات':'Services' }}</option>
                    <option value="payment" {{ old('category')=='payment'?'selected':'' }}>{{ app()->getLocale()==='ar'?'الدفع':'Payment' }}</option>
                    <option value="technical" {{ old('category')=='technical'?'selected':'' }}>{{ app()->getLocale()==='ar'?'تقني':'Technical' }}</option>
                </select>
            </div>
            <div>
        ...
        <div style="display:flex;justify-content:flex-end;gap:12px;">
            ...
            <button type="submit" class="btn-adm"><i class="fas fa-save"></i> {{ app()->getLocale()==='ar'?'حفظ':'Save' }}</button>
        </div>
    </form>
</div>
@endsection
